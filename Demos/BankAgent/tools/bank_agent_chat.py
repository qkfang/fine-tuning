...
Tests the Microsoft Foundry bank agent with various scenarios.
...
class BankAgentTester:
    """Test the bank agent with predefined scenarios."""
    ...
    def __init__(self, endpoint=None, model_name=None, mcp_server_url=None):
        ...
        self.system_prompt = """You are a helpful bank customer service agent. 
Help customers with their orders, returns, and account information."""
    ...
    def test_connection(self) -> bool:
        ...
        if not self.mcp_server_url:
            print(f"{Fore.RED}[X] Missing MCP_SERVER_URL{Style.RESET_ALL}")
            return False
        ...
        try:
            ...
            print(f"{Fore.GREEN}[OK] Connected to Microsoft Foundry{Style.RESET_ALL}")
            return True
        except Exception as e:
    ...
    def test_agent_creation(self) -> bool:
        ...
        if not self.project_client:
            print(f"{Fore.RED}[X] Project client not initialized{Style.RESET_ALL}")
            return False
        ...
        try:
            ...
            print(f"{Fore.GREEN}[OK] Agent created: {self.agent.id}{Style.RESET_ALL}")
            print(f"  Model: {self.model_name}")
            return True
        except Exception as e:
            print(f"{Fore.RED}[X] Agent creation failed: {e}{Style.RESET_ALL}")
            return False
    ...
    def test_thread_creation(self) -> bool:
        ...
        try:
            ...
            print(f"{Fore.GREEN}[OK] Thread created: {self.thread.id}{Style.RESET_ALL}")
            return True
        except Exception as e:
            print(f"{Fore.RED}[X] Thread creation failed: {e}{Style.RESET_ALL}")
            return False
    ...
    def test_simple_query(self) -> bool:
        ...
        if not self.project_client or not self.agent or not self.thread:
            print(f"{Fore.RED}[X] Prerequisites not met{Style.RESET_ALL}")
            return False
        ...
        try:
            ...
            if run.status == "completed":
                    ...
                    if message.role == "assistant":
                        for content in message.content:
                            if hasattr(content, 'text'):
                                ...
                                print(f"{Fore.GREEN}[OK] Got response{Style.RESET_ALL}")
                ...
                print(f"{Fore.RED}[X] Run failed with status: {run.status}{Style.RESET_ALL}")
                if hasattr(run, 'last_error') and run.last_error:
                    print(f"{Fore.RED}  Error: {run.last_error}{Style.RESET_ALL}")
                return False
                
        except Exception as e:
            print(f"{Fore.RED}[X] Query test failed: {e}{Style.RESET_ALL}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def test_user_lookup(self) -> bool:
        ...
        try:
            ...
            if run.status == "completed":
                ...
                if tool_called:
                    print(f"{Fore.GREEN}[OK] User lookup successful (tool was called){Style.RESET_ALL}")
                    return True
                else:
            ...
            print(f"{Fore.RED}[X] User lookup test failed: {e}{Style.RESET_ALL}")
            return False
    ...
    def run_all_tests(self, notebook_mode: bool = False) -> dict:
        ...
        if notebook_mode:
            ...
            if passed == total:
                print("\nAll tests passed! Bank agent is fully operational.")
            elif passed > 0:
            ...
            if passed == total:
                ...
                print(f"{Fore.RED}[X] All tests failed{Style.RESET_ALL}")
...
def main():
    ...
    print(f"{Fore.CYAN}Bank Agent Test Suite{Style.RESET_ALL}\n")
